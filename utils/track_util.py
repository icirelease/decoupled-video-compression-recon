...
def load_mot(detections, nms_overlap_thresh=None, with_classes=True, nms_per_class=False):
    ...
    if type(detections) is str:
        ...
        #raw = np.genfromtxt(detections, delimiter=',', dtype=np.uint32)
        if np.isnan(raw).all():
            raw = np.genfromtxt(detections, delimiter=' ', dtype=np.float32)
            #raw = np.genfromtxt(detections, delimiter=' ', dtype=np.uint32)
        ...
        #raw = detections.astype(np.uint32)
...
def compute_from_to(track,object_class):
    box_list = []
    for item in track['bboxes']:
        box_list.append(list(item))
    for i in range(len(box_list)):
        for j in range(len(box_list[i])):
            box_list[i][j] = int(box_list[i][j])
            box_list[i][j] += 1
    from_bbox = box_list[int(len(track['bboxes'])/2)]
    from_box_frame = int(track['start_frame'] + len(track['bboxes'])/2)
    csv_list = []
    for i, bbox in enumerate(box_list):
        #print(bbox,' frame:',track['start_frame'] + i)
        bbox_frame = track['start_frame'] + i
        if object_class == 'vehicle':
            from_info = str(from_box_frame)+'/'+str(from_bbox[0])+'_'+str(from_bbox[1])+'_'+str(from_bbox[2])+'_'+str(from_bbox[3])+'.jpg'
            to_info = str(bbox_frame)+'/'+str(bbox[0])+'_'+str(bbox[1])+'_'+str(bbox[2])+'_'+str(bbox[3])+'.jpg'
        else:
            from_info = str(from_box_frame)+'/'+str(from_bbox[0])+'_'+str(from_bbox[1])+'_'+str(from_bbox[2])+'_'+str(from_bbox[3])+'.png'
            to_info = str(bbox_frame)+'/'+str(bbox[0])+'_'+str(bbox[1])+'_'+str(bbox[2])+'_'+str(bbox[3])+'.png'
            #from_info = str(from_box_frame)+'/'+str(from_bbox[0])+'_'+str(from_bbox[1])+'_'+str(from_bbox[2])+'_'+str(from_bbox[3])+'.jpg'
            #to_info = str(bbox_frame)+'/'+str(bbox[0])+'_'+str(bbox[1])+'_'+str(bbox[2])+'_'+str(bbox[3])+'.jpg'
        csv_list.append({'from':from_info,'to':to_info})
    df =pd.DataFrame(csv_list,columns = ['from','to'])
    return df


def save_to_csv(out_path, tracks,object_class, fmt='motchallenge'):
    """
    ...
    with open(out_path, "w") as ofile:
        if fmt == 'motchallenge':
            ...
            field_names = ['frame', 'id', 'x0', 'y0', 'x1', 'y1', 'score', 'wx', 'wy', 'wz']
        elif fmt == 'visdrone':
            field_names = ['frame', 'id', 'x0', 'y0', 'x1', 'y1', 'score', 'object_category', 'truncation', 'occlusion']
        else:
            raise ValueError("unknown format type '{}'".format(fmt))

        #odict = csv.DictWriter(ofile, field_names)
        #id_ = 1
        track_df = pd.DataFrame(columns = ['from','to'])
        for track in tracks:
            curr_track_df = compute_from_to(track,object_class)
            track_df = pd.concat([track_df,curr_track_df],axis = 0,ignore_index= True)
